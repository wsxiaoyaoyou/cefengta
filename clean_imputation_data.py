...
def shear_interpolatin(data, ID_can, height, yuan_ID_can_id, start_time, end_time):
    ...
    return Result[['Date_Time', ID_can]]

def shear_interpolatin_max(data, ID_can, height, start_time, end_time):
    # 每一个时刻的风切变，随机性太强
    # data_can = data[cans.split(',')]
    # data[ID_can] = data_can.apply(lambda x: cal_shear_every(x, height, yuan_ID_can_id), axis=1)
    # 计算总体的风切变
    c, a = cal_shear_all(data, height)
    cal_data = data[['Date_Time', ID_can, 'max_WS_AVG', 'max_height']]
    cal_data[ID_can] = cal_data[ID_can].replace(' ', np.nan).replace('None', np.nan).astype('float')
    cal_data['max_WS_AVG'] = cal_data['max_WS_AVG'].replace(' ', np.nan).replace('None', np.nan).astype('float')
    Result = pd.DataFrame()
    Result['Date_Time'] = pd.date_range(start=start_time, end=end_time, freq='10T')
    Result['Date_Time'] = Result['Date_Time'].apply(lambda x: x.strftime('%Y-%m-%d %H:%M:%S'))
    Result = pd.merge(Result, cal_data, how='left', on='Date_Time')
    Result[ID_can] = Result.apply(lambda x: cal_shear_data(x[ID_can], int(ID_can.split('_')[0]), x['max_WS_AVG'], x['max_height'], a), axis=1)
    Result[ID_can] = Result[ID_can].apply(lambda x: np.around(x, 3))
    return Result[['Date_Time', ID_can]]


def cal_new_wind(x, height):
    for i in range(1, len(height)):
        if not np.isnan(x[str(height[i]) + '_WS_AVG']):
            return x[str(height[i]) + '_WS_AVG']
    return np.nan

def cal_new_wind_height(x, height):
    for i in range(1, len(height)):
        if not np.isnan(x[str(height[i]) + '_WS_AVG']):
            return height[i]
    return np.nan

def clean_imputation_data(cefengta_id):
    ...
    # 这里开始处理插补 只针对风速AVG
    height = [int(x.split('_')[0]) for x in WS_list]
    height = list(set(height))
    height.sort(reverse=True)
    data['max_WS_AVG'] = data.apply(lambda x: cal_new_wind(x, height), axis=1)
    data['max_height'] = data.apply(lambda x: cal_new_wind_height(x, height), axis=1)
    height_list = height.copy() #一定要有不然顺序会乱
    for index, height_i in enumerate(height_list):
        if height_i == height_list[0]:
            data_ID_can = shear_interpolatin_max(data, str(height_i) + '_WS_AVG', height, data['Date_Time'].min(),
                                                 data['Date_Time'].max())
            data.drop(str(height_i) + '_WS_AVG', inplace=True, axis=1)
            data.drop(['max_WS_AVG', 'max_height'], inplace=True, axis=1)
            data = pd.merge(data, data_ID_can, on='Date_Time')
        else:
            data_ID_can = shear_interpolatin(data, str(height_i) + '_WS_AVG', height, height_list[index - 1],
                                             data['Date_Time'].min(), data['Date_Time'].max())
            data.drop(str(height_i) + '_WS_AVG', inplace=True, axis=1)
            data = pd.merge(data, data_ID_can, on='Date_Time')
    # 订正 最高层 原始通道有没有vert z方向 垂直 不做订正
    if str(height_list[0]) + '_ZWS_AVG' in data.columns:
        conn = pymysql.connect(host=host, port=port, user=user, password=password, database=database, charset='utf8mb4')
        cursor = conn.cursor()
        select_data = "SELECT ORIGINCHANNEL FROM cefengta.channel_configuration where USEDCHANNEL = '%s' and ID = '%s';" % (
        str(height_list[0]) + '_ZWS_AVG', cefengta_id)
        cursor.execute(select_data)
        values = cursor.fetchone()
        if values:
            ORIGINCHANNEL = values[0]
        else:
            ORIGINCHANNEL = ''
        cursor.close()
        conn.close()
        if ('vert' not in ORIGINCHANNEL) & ('z方向' not in ORIGINCHANNEL) & ('垂直' not in ORIGINCHANNEL):
            data[str(height_list[0]) + '_ZWS_AVG'] = data[str(height_list[0]) + '_ZWS_AVG'].replace(' ', np.nan).replace('None', np.nan).astype('float')
            ZWS_mean = np.nanmean(data[str(height_list[0]) + '_ZWS_AVG'])
            WS_mean = np.nanmean(data[str(height_list[0]) + '_WS_AVG'])
            if ZWS_mean > WS_mean:
                data[str(height_list[0]) + '_WS_AVG'] = data[str(height_list[0]) + '_WS_AVG'] + (ZWS_mean - WS_mean)
            else:
                data[str(height_list[0]) + '_ZWS_AVG'] = data[str(height_list[0]) + '_ZWS_AVG'] + (WS_mean - ZWS_mean)
    # 这里在插入数据
...
if __name__ == '__main__':
    ...
    if '67E3-17ED' in result.stdout.decode('utf-8'):
        ...
        clean_imputation_data(cefengta_id)