...
def cal_wp_yue(data, wind_name, time_name):
    '''
    计算月均风速
    :param data: pd.Dataframe
    :param wind_name: 风速列名
    ...
    for i in ['01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12']:
        try:
            monthly_data = data[data['time'] == i][wind_name]
            wp = np.nanmean(0.5 * (monthly_data ** 3))  # 注意用** 表示幂运算
            result.append(round(wp, 2))  # 保留两位小数
        except:
            result.append(np.nan)
    return result
